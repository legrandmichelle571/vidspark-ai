/**
 * VidSpark AI — Cloudflare Worker Proxy
 * ─────────────────────────────────────
 ...
 *                      wrangler secret put VIDSPARK_SECRET
 */
  ...
  async fetch(request, env, ctx) {
    ...
    if (!quotaOk) {
      return corsResponse({
        ...
        plan: user.plan,
        upgrade_url: "https://vidspark.ai/pricing"
      }, 429, env);
    }
    ...
    const cached = await env.VIDSPARK_CACHE.get(cacheKey);
    if (cached) {
    ...
    ctx.waitUntil(
      env.VIDSPARK_CACHE.put(cacheKey, JSON.stringify(result), {
...
async function validateToken(token, env) {
  if (token === "free_demo_token") {
    return { id: "demo", plan: "free", email: "demo@vidspark.ai" };
  }

  try {
    const userData = await env.VIDSPARK_CACHE.get(`user:${token}`);
...
async function checkAndDecrementQuota(user, env) {
  ...
  let count = 0;
  try {
    const stored = await env.VIDSPARK_CACHE.get(key);
    count = stored ? parseInt(stored) : 0;
  } catch {}

  if (count >= limits.daily) return false;

  await env.VIDSPARK_CACHE.put(key, String(count + 1), {
    expirationTtl: 86400
  });
