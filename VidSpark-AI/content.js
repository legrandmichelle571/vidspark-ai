/**
 * VidSpark AI v4.6 — Content Script
 * Auth header: [Connexion] [Inscription] visible dans le header
 * buildHeaderAuth() — sans backticks imbriqués
 */
"use strict";

/* ══════════════════════════════════════════════════════════════
   ADAPTIVE COLOR — Extrait les couleurs de la miniature YouTube
   et applique un thème dynamique sur toute l'interface
══════════════════════════════════════════════════════════════ */
function extractThumbnailColors(videoId, callback) {
  if (!videoId) { callback(null); return; }
  const img = new Image();
  ...
  img.onload = function() {
    try {
      ...
      canvas.width = 40; canvas.height = 22;
      const ctx = canvas.getContext("2d");
      ctx.drawImage(img, 0, 0, 40, 22);
      const data = ctx.getImageData(0, 0, 40, 22).data;

      /* Calcul couleur dominante par moyenne pondérée */
      ...
      const factor = max > 0 ? Math.min(2.2, 180/max) : 1;
      ...
      callback({ r, g, b,
        ...
        glass:   `rgba(${Math.round(r*0.12)},${Math.round(g*0.10)},${Math.round(b*0.15)},0.50)`,
        border:  `rgba(${r},${g},${b},0.30)`,
        cardBg:  `rgba(${Math.round(r*0.08)},${Math.round(g*0.07)},${Math.round(b*0.12)},0.48)`,
      });
...
function applyAdaptiveColor(colors) {
  const panel = document.getElementById("echo-rank-panel");
  if (!panel || !colors) return;
  const {r,g,b} = colors;
  panel.style.setProperty("--accent",      `rgba(${r},${g},${b},1)`);
  panel.style.setProperty("--accent-rgb",  `${r},${g},${b}`);
  panel.style.setProperty("--glow",        `rgba(${r},${g},${b},0.28)`);
  panel.style.setProperty("--glass-bg",    colors.glass);
  panel.style.setProperty("--card-bg",     colors.cardBg);
  panel.style.setProperty("--card-border", colors.border);
  panel.style.setProperty("--card-hover",  `rgba(${r},${g},${b},0.22)`);

  /* Lueur externe sur le panel */
  panel.style.boxShadow =
    `0 8px 40px rgba(0,0,0,0.5), 0 0 0 1px ${colors.border}, 0 0 60px ${colors.glow}`;
}

/* ── Configuration URLs ─────────────────────────────────────
   Remplacer par votre URL Stripe Checkout ou page pricing
   Exemple: "https://buy.stripe.com/xxxxx" ou "https://votre-site.com/pricing"
──────────────────────────────────────────────────────────── */
/* UPGRADE_URL est géré dans background.js > CONFIG.UPGRADE_URL */
...
const I18N = {
  ...
  btn_viral_ai:"✨ Virale AI Analyse",format_standard:"Format 16:9 standard",
    ...
    btn_logout:"Déconnexion",auth_greeting:"Bonjour",
    auth_not_connected:"Non connecté",
    auth_free_plan:"Plan Gratuit — 5 analyses/jour",
    auth_pro_plan:"Plan Pro — Analyses illimitées",
    auth_business_plan:"Plan Business",
  ...
  en:{
    ...
    btn_logout:"Sign out",auth_greeting:"Hello",
    auth_not_connected:"Not connected",
    auth_free_plan:"Free Plan — 5 analyses/day",
    auth_pro_plan:"Pro Plan — Unlimited",
    auth_business_plan:"Business Plan",
  ...
  ar:{
    ...
    btn_logout:"تسجيل الخروج",auth_greeting:"مرحباً",
    auth_not_connected:"غير متصل",
    auth_free_plan:"الخطة المجانية — 5 تحليلات/يوم",
    auth_pro_plan:"خطة Pro — غير محدود",
    auth_business_plan:"خطة Business",
  ...
  es:{
    ...
    btn_logout:"Cerrar sesión",auth_greeting:"Hola",
    auth_not_connected:"No conectado",
    auth_free_plan:"Plan Gratuito — 5 análisis/día",
    auth_pro_plan:"Plan Pro — Ilimitado",
    auth_business_plan:"Plan Business",
  ...
  de:{
    ...
    btn_logout:"Abmelden",auth_greeting:"Hallo",
    auth_not_connected:"Nicht verbunden",
    auth_free_plan:"Free-Plan — 5 Analysen/Tag",
    auth_pro_plan:"Pro-Plan — Unbegrenzt",
    auth_business_plan:"Business-Plan",
  ...
  pt:{
    ...
    btn_logout:"Sair",auth_greeting:"Olá",
    auth_not_connected:"Não conectado",
    auth_free_plan:"Plano Gratuito — 5 análises/dia",
    auth_pro_plan:"Plano Pro — Ilimitado",
    auth_business_plan:"Plano Business",
  ...
  it:{
    ...
    btn_login:"Accedi",btn_register:"Crea account",
    btn_logout:"Esci",auth_greeting:"Ciao",
    auth_not_connected:"Non connesso",
    auth_free_plan:"Piano Gratuito — 5 analisi/giorno",
    auth_pro_plan:"Piano Pro — Illimitato",
    auth_business_plan:"Piano Business",
...
I18N.ru = {
  ...
  btn_login:"Войти",btn_register:"Создать аккаунт",btn_logout:"Выйти",auth_greeting:"Привет",auth_not_connected:"Не подключён",auth_free_plan:"Free — 5 анализов/день",auth_pro_plan:"Pro — безлимит",auth_business_plan:"Business",
...
I18N.ja = {
  ...
  btn_login:"ログイン",btn_register:"アカウント作成",btn_logout:"ログアウト",auth_greeting:"こんにちは",auth_not_connected:"未接続",auth_free_plan:"無料 — 5回/日",auth_pro_plan:"Pro — 無制限",auth_business_plan:"Business",
...
I18N.ko = {
  ...
  btn_login:"로그인",btn_register:"회원가입",btn_logout:"로그아웃",auth_greeting:"안녕하세요",auth_not_connected:"연결 안됨",auth_free_plan:"무료 — 5회/일",auth_pro_plan:"Pro — 무제한",auth_business_plan:"Business",
...
I18N.hi = {
  ...
  btn_login:"लॉगिन",btn_register:"खाता बनाएं",btn_logout:"लॉगआउट",auth_greeting:"नमस्ते",auth_not_connected:"कनेक्ट नहीं",auth_free_plan:"फ्री — 5/दिन",auth_pro_plan:"Pro — असीमित",auth_business_plan:"Business",
...
I18N.zh = {
  ...
  btn_login:"登录",btn_register:"注册",btn_logout:"退出",auth_greeting:"你好",auth_not_connected:"未连接",auth_free_plan:"免费版 — 5次/天",auth_pro_log:"Pro版 — 无限制",auth_business_plan:"Business版",
...
I18N.tr = {
  ...
  btn_login:"Giriş yap",btn_register:"Hesap oluştur",btn_logout:"Çıkış",auth_greeting:"Merhaba",auth_not_connected:"Bağlı değil",auth_free_plan:"Ücretsiz — 5/gün",auth_pro_plan:"Pro — Sınırsız",auth_business_plan:"Business",
...
let currentPlan     = "free";   /* free | pro | business — vérifié côté serveur */
let currentUser     = null;     /* userData depuis le backend */
let currentMonthlyUsage = 0;    /* Utilisation mensuelle actuelle (FREE only) */
let currentMonthlyLimit = 10;   /* Limite mensuelle du plan (10 FREE, 500 PRO, 10k BUSINESS) */
let panelMounted    = false;
let lastUrl         = location.href;
let activeTab       = "overview";
/* Cache analyses pour performance — évite recalculs répétés */
const _analysisCache = new Map();
function getCachedAnalysis(videoId){return _analysisCache.get(videoId)||null;}
function setCachedAnalysis(videoId,data){
  if(_analysisCache.size>20)_analysisCache.clear(); /* Limite mémoire */
  _analysisCache.set(videoId,data);
}
...
══════════════════════════════════════════════════════════════ */
/* ══════════════════════════════════════════════════════════════
   DÉTECTION AUTOMATIQUE DE LANGUE DE LA VIDÉO
   Basée sur le titre + description de la vidéo
══════════════════════════════════════════════════════════════ */
function detectVideoLanguage(title, description){
  const text = (title||"") + " " + (description||"").slice(0,200);

  /* Arabe : lettres unicode [؀-ۿ] */
  const arabicChars = (text.match(/[؀-ۿ]/g)||[]).length;
  /* Français : lettres accentuées + mots courants */
  const frenchWords = /(le|la|les|de|du|un|une|des|est|et|en|sur|pour|que|qui|dans|il|elle|nous|vous|ils|elles|je|tu|on|avec|par|mais|ou|donc|car|ni|or|comment|pourquoi|guide|tuto|gratuit|débutant|complet|incroyable|révélé|secret)/i;
  const frenchScore = (text.match(frenchWords)||[]).length;
  /* Anglais : mots courants */
  const englishWords = /(the|a|an|is|are|was|were|be|been|have|has|had|do|does|did|will|would|can|could|should|may|might|must|how|why|what|when|where|who|which|this|that|these|those|to|for|of|in|on|at|by|with|from|up|out|if|then|than|or|and|not|but|free|guide|tutorial|beginner|ultimate|amazing|secret|best|top)/i;
  ...
  /* Fallback: langue de l'interface */
  return currentLanguage || "fr";
}

/* Mots puissants SEO par langue de vidéo */
const POWER_WORDS = {
  ar: ["صادم","خطير","مفاجأة","السر","لن تصدق","عاجل","مذهل","حصري","أسرار","مجاني","كامل","مبتدئ","الأفضل","رائع","ترند","كيف","لماذا","درس","شرح","طريقة"],
  fr: ["incroyable","secret","révélé","gratuit","guide complet","débutant","ultime","pourquoi","comment","astuce","meilleur","facile","rapide","efficace","méthode","tuto","explication","top","découverte","exclusif"],
  en: ["amazing","secret","ultimate","free","beginner","guide","how","why","best","top","revealed","exclusive","complete","easy","fast","effective","method","discover","proven","insider"],
  default: ["amazing","best","top","ultimate","secret","viral","free","gratuit","incroyable","comment","pourquoi","مذهل","الأفضل","رائع"]
};

/* Hooks CTR par langue de vidéo */
const CTR_HOOKS = {
  ar: [/^(كيف|لماذا|ما هو|هل|اكتشف|شاهد|\d)/i, /[؟?]/],
  fr: [/^(comment|pourquoi|quel|quelle|découvrez|voici|\d)/i, /[?!]/],
  en: [/^(how|why|what|discover|here|watch|\d)/i, /[?!]/],
};

/* Mots-clés manquants par langue */
const MISSING_KW = {
  ar: ["درس","مجاني","كامل","شرح","طريقة","مبتدئ","2024","تعلم","خطوة","سهل"],
  fr: ["tuto","gratuit","complet","guide","débutant","2024","explication","méthode","facile","astuce"],
  en: ["tutorial","free","complete","guide","beginner","2024","how to","step by step","easy","tips"],
  default: ["tuto","gratuit","complet","guide","débutant","2024"]
};
...
function computeSEOScore(title,descLength){
  ...
  /* Mots puissants adaptés à la langue de la vidéo */
  const vidLang=detectVideoLanguage(title,"");
  const pw=POWER_WORDS[vidLang]||POWER_WORDS.default;
  let pwc=0; pw.forEach(w=>{if(title.toLowerCase().includes(w.toLowerCase()))pwc++;});
  s+=Math.min(pwc*4,12);
  /* Hooks CTR adaptés */
  const hookPatterns=CTR_HOOKS[vidLang]||[/^(comment|how|كيف)/i,/[?!]/];
  const hasHook=hookPatterns.some(p=>p.test(title));
  if(hasHook)s+=5;
...
function computeViralScore(title,descLength,seo){
  const vidLang2=detectVideoLanguage(title,"");
  const hookPats=CTR_HOOKS[vidLang2]||[/^(comment|how|كيف)/i,/[?!]/];
  const hasHook=hookPats.some(p=>p.test(title));
  const hasNum=/\d/.test(title);
  const emotWords=POWER_WORDS[vidLang2]||POWER_WORDS.default;
  const hasEmot=emotWords.some(w=>title.toLowerCase().includes(w.toLowerCase()));
...
function computeViralPotential(title,descLength,seo){
  ...
  const vidLang3=detectVideoLanguage(title,"");
  const hookPats3=CTR_HOOKS[vidLang3]||[/^(comment|how|كيف)/i,/[?!]/];
  const hasHook=hookPats3.some(p=>p.test(title));
  const hasNum=/\d/.test(title);
  const emotW=POWER_WORDS[vidLang3]||POWER_WORDS.default;
  const hasEmot=emotW.some(w=>title.toLowerCase().includes(w.toLowerCase()));
...
function buildChecklist(title,descLength,description){
  const L=I18N[currentLanguage]||I18N.en;
  /* Détecter la langue de la vidéo pour des suggestions contextuelles */
  const vidLang=detectVideoLanguage(title,description||"");
...
function sendBG(payload){
  return new Promise((resolve,reject)=>{
    chrome.runtime.sendMessage({type:"VIDSPARK_API_REQUEST",payload},r=>{
      if(chrome.runtime.lastError)return reject(new Error(chrome.runtime.lastError.message));
      if(r?.error){
        /* Erreurs spéciales */
        if(r.error==="UPGRADE_REQUIRED"){showPremiumGate();return reject(new Error(r.error));}
        if(r.error==="QUOTA_EXCEEDED"){showToast("⚠ "+T("quota_exceeded")||"Quota journalier atteint");return reject(new Error(r.error));}
        if(r.error==="Not authenticated"){showAuthRequired();return reject(new Error(r.error));}
...
/* ── Afficher l'interface de connexion requise ── */
function showAuthRequired(){
  const existing=document.getElementById("echo-auth-required");
  if(existing)existing.remove();
  const el=document.createElement("div");
  el.id="echo-auth-required";
  el.innerHTML=`
    <div class="echo-gate-overlay" id="echoAuthOverlay">
      <div class="echo-gate-box">
        <div class="echo-gate-icon">🔐</div>
        <div class="echo-gate-title">Connexion requise</div>
        <div class="echo-gate-msg">Connectez-vous pour utiliser les fonctionnalités IA d'VidSpark AI.</div>
        <button class="echo-gate-btn" id="btnOpenLogin">Se connecter</button>
        <button class="echo-gate-close" id="btnAuthClose">✕ Plus tard</button>
      </div>
    </div>`;
  document.body.appendChild(el);
  el.querySelector("#btnAuthClose").addEventListener("click",()=>el.remove());
  el.querySelector("#echoAuthOverlay").addEventListener("click",e=>{if(e.target.id==="echoAuthOverlay")el.remove();});
  el.querySelector("#btnOpenLogin").addEventListener("click",()=>{el.remove();showLoginModal();});
}

/* ── Modal de connexion ── */
/* ── Authentification 100% Google ── */
function showLoginModal(){
  const existing=document.getElementById("echo-login-modal");
  if(existing)existing.remove();
  const el=document.createElement("div");
  el.id="echo-login-modal";
  el.innerHTML=`
    <div class="echo-gate-overlay" id="echoLoginOverlay">
      <div class="echo-login-card">
        <!-- Fermer -->
        <button class="echo-login-close-x" id="btnLoginClose">✕</button>

        <!-- Logo -->
        <div class="echo-login-logo">
          <span style="font-size:28px">⚡</span>
          <div>
            <div style="font-size:16px;font-weight:800;color:#c4b5fd">VidSpark AI</div>
            <div style="font-size:10px;color:#666">YouTube SEO Intelligence</div>
          </div>
        </div>

        <!-- Texte intro -->
        <div style="text-align:center;margin:16px 0;font-size:13px;color:#aaa">
          Authentifiez-vous avec votre compte Google
        </div>

        <!-- Bouton Google uniquement -->
        <button class="echo-google-btn" id="btnGoogleLogin">
        ...
        <!-- Message erreur -->
        <div id="echo-login-error" class="echo-login-error" style="display:none"></div>

        <!-- CGU -->
        <div class="echo-login-cgu" style="margin-top:20px">
          En continuant, vous acceptez les
          <a href="https://vidspark.ai/terms" target="_blank" style="color:#7c6dfa">CGU</a> et la
          <a href="https://vidspark.ai/privacy" target="_blank" style="color:#7c6dfa">Confidentialité</a>
        </div>
      </div>
    </div>`;
  document.body.appendChild(el);

  el.querySelector("#btnLoginClose").addEventListener("click",()=>el.remove());
  el.querySelector("#echoLoginOverlay").addEventListener("click",e=>{if(e.target.id==="echoLoginOverlay")el.remove();});

  /* Connexion Google */
  el.querySelector("#btnGoogleLogin").addEventListener("click",()=>{
    const btn=el.querySelector("#btnGoogleLogin");
    const errEl=document.getElementById("echo-login-error");
    btn.disabled=true;
    btn.innerHTML='<div class="echo-spin-sm"></div> Connexion Google...';
    errEl.style.display="none";

    chrome.runtime.sendMessage({type:"VIDSPARK_AUTH",payload:{action:"google"}},r=>{
      if(chrome.runtime.lastError){
        errEl.textContent="Erreur de communication avec l'extension.";
...
function openFullReport(){
  ...
  if(!data||!data.videoId){
    showToast(T("error_no_video")||"Ouvrez une vidéo YouTube d'abord");
    return;
  }
  ...
  try{
    ...
    console.error("[VidSpark] Rapport erreur:",err);
    showToast("Erreur rapport: "+err.message);
  }
}

/* ── Modal Premium Gate (utilisateurs Free) — v5.0 ── */
function showPremiumGate(){
  const existing=document.getElementById("echo-premium-gate");
  if(existing)existing.remove();

  const container=document.createElement("div");
  container.id="echo-premium-gate";
  container.innerHTML=`
    <div class="echo-premium-overlay" id="echoGateOverlay"></div>
    <div class="echo-premium-popup">
      <button class="close-btn" id="btnGateClose">×</button>
      <h2>🚀 ${T("upgrade_to_pro")||"Upgrade to PRO"}</h2>
      <p>${T("unlock_premium")||"Unlock unlimited AI analysis and advanced features"}</p>
      <div class="price-tag">$9.99<span style="font-size: 14px; color: #8b949e;">/month</span></div>
      <p style="font-size: 12px; color: #8b949e;">Billed monthly or $95.90/year</p>
      <div style="margin: 16px 0; padding: 12px; background: rgba(0,212,255,0.1); border-left: 3px solid #00d4ff; border-radius: 4px;">
        <p style="margin: 4px 0; font-size: 12px;">✓ 500 analyses/month</p>
        <p style="margin: 4px 0; font-size: 12px;">✓ 100 AI reports/month</p>
        <p style="margin: 4px 0; font-size: 12px;">✓ Unlimited AI titles</p>
        <p style="margin: 4px 0; font-size: 12px;">✓ Export PDF & More</p>
      </div>
      <button class="echo-gate-btn" id="btnGateUpgrade" style="width: 100%; padding: 12px; background: linear-gradient(90deg, #00d4ff, #0099cc); color: #000; border: none; border-radius: 8px; cursor: pointer; font-weight: 600; font-size: 14px; margin-top: 16px;">
        ${T("upgrade_btn")||"View Pricing"}
      </button>
    </div>`;

  document.body.appendChild(container);

  const overlay=container.querySelector("#echoGateOverlay");
  const closeBtn=container.querySelector("#btnGateClose");
  const upgradeBtn=container.querySelector("#btnGateUpgrade");
  ...
  upgradeBtn.addEventListener("click", ()=>{
    container.remove();
    chrome.runtime.sendMessage({ type: "VIDSPARK_GET_CONFIG" }, cfg => {
...
/* ── Helper: Barre de quota mensuel pour utilisateurs FREE ── */
function buildQuotaBar(){
  if(currentPlan!=="free") return ""; /* Seulement pour FREE */
  const percent=Math.min(100, (currentMonthlyUsage/currentMonthlyLimit)*100);
  const statusClass=percent>80?"critical":(percent>60?"warning":"");
  return `
    <div style="padding: 8px 12px; margin-top: 8px; border-top: 1px solid var(--border);">
      <div class="echo-quota-text">Analyses: ${currentMonthlyUsage}/${currentMonthlyLimit} this month</div>
      <div class="echo-quota-bar">
        <div class="echo-quota-bar-fill" style="width: ${percent}%"></div>
      </div>
      ${percent>=currentMonthlyLimit?`<div class="echo-quota-text critical">💾 Quota full — Upgrade to PRO</div>`:
        percent>80?`<div class="echo-quota-text warning">⚠ ${Math.floor(currentMonthlyLimit-currentMonthlyUsage)} analyses left</div>`:""}
    </div>
  `;
}
...
function renderOverview(data,scores,checklist){
  ...
  /* Barre auth aperçu — Google OAuth uniquement */
  const _authBar = (()=>{
    if(!currentUser){
      return '<div class="echo-auth-bar-ov">'
        +'<span class="echo-auth-bar-ov-txt">'+(T("auth_not_connected")||"Authentification Google requise")+'</span>'
        +'<div class="echo-auth-bar-ov-btns">'
        +'<button class="echo-hdr-login" id="btnOvLogin" style="flex:1">'
        +'<svg width="14" height="14" viewBox="0 0 48 48" style="margin-right:6px;display:inline-block"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.18 1.48-4.97 2.31-8.16 2.31-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>'
        +'Continuer avec Google'
        +'</button>'
        +'</div></div>';
    }
...
function renderThumbnail(data,scores){
  ...
  return `
    ...
    <div class="echo-card echo-emotions-card">
    ...
    <div class="echo-card echo-suggestions-card">
...
function renderCompetitor(data,scores){
  const keywords=(data.title||"").split(/\s+/).filter(w=>w.length>3);
  /* Mots-clés manquants adaptés à la langue de la vidéo */
  const vidLangComp=detectVideoLanguage(data.title, data.description||"");
  const kwList=MISSING_KW[vidLangComp]||MISSING_KW.default;
  const missing=kwList.filter(w=>w&&!data.title.toLowerCase().includes(w.toLowerCase()));
  return `
    <div class="echo-card">
      <div class="echo-card-head">${T("comp_position")}</div>
      <div class="echo-scores-row">
        <div class="echo-score-card">
          <div class="echo-score-num" style="color:${scoreColor(scores.seo)}">${scores.seo}</div>
          <div class="echo-score-lbl">${T("score_seo")}</div>
          <div class="echo-score-pot">↑${computeSEOPotential(data.title,data.descLength)}</div>
        </div>
        <div class="echo-score-card">
          <div class="echo-score-num" style="color:${scoreColor(scores.viral)}">${scores.viral}</div>
          <div class="echo-score-lbl">${T("score_viral")}</div>
          <div class="echo-score-pot">↑${computeViralPotential(data.title,data.descLength,scores.seo)}</div>
        </div>
        <div class="echo-score-card">
          <div class="echo-score-num" style="color:${scoreColor(scores.thumb)}">${scores.thumb}</div>
          <div class="echo-score-lbl">${T("score_thumb")}</div>
          <div class="echo-score-pot">↑${computeThumbPotential(scores.thumb)}</div>
        </div>
      </div>
    </div>
    <div class="echo-card">
...
function renderActions(data){
  return `
    <div class="echo-actions-grid">
      <button class="echo-action-btn blue"   id="btnCopyTitle">📋 ${T("act_copy_title")}</button>
      <button class="echo-action-btn red"    id="btnGenDesc">✍️ ${T("act_description")}</button>
      <button class="echo-action-btn purple" id="btnGenTags">🏷️ ${T("act_tags")}</button>
      <button class="echo-action-btn green"  id="btnOpenThumb">🖼️ ${T("act_thumbnail")}</button>
    </div>
    <div id="card-action-result" style="display:none">
      <div class="echo-card" style="margin-top:8px">
        ...
        <button class="echo-action-btn" id="btnCopyResult" style="background:var(--surface2);color:var(--muted);margin-top:8px;font-size:12px">📋 Copier</button>
      </div>
    </div>
    <button class="echo-full-btn" id="btnFullReport2" style="margin-top:8px">✨ ${T("act_full_report")}</button>
  `;
}

/* ══════════════════════════════════════════════════════════════
   PAYWALL INITIALIZATION - Premium Feature Access Control
══════════════════════════════════════════════════════════════ */

let paywall_initialized = false;

function initPaywall() {
  if (paywall_initialized) return;

  // Get user plan from chrome storage (set by background.js)
  chrome.storage.local.get(['userPlan'], (result) => {
    const userPlan = result.userPlan || 'free';
    window.VIDSPARK_USER_PLAN = userPlan;

    console.log('[VidSpark Paywall] Initializing for plan:', userPlan);

    // Wait for Paywall object to be available (loaded by manifest)
    const checkPaywall = setInterval(() => {
      if (window.Paywall && typeof window.Paywall.init === 'function') {
        clearInterval(checkPaywall);

        // Initialize paywall based on user plan
        window.Paywall.init(userPlan);
        paywall_initialized = true;

        console.log('[VidSpark Paywall] Initialized successfully for', userPlan);
      }
    }, 100);

    // Safety timeout
    setTimeout(() => clearInterval(checkPaywall), 5000);
  });
}

// Listen for plan updates from background.js
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'PLAN_UPDATED') {
    console.log('[VidSpark Paywall] Plan updated to:', request.plan);
    window.VIDSPARK_USER_PLAN = request.plan;

    if (window.Paywall && typeof window.Paywall.init === 'function') {
      window.Paywall.init(request.plan);
    }
  }
});
...
function createPanel(){
  ...
  let cachedScores = getCachedAnalysis(data.videoId);
  const seoScore    = cachedScores?.seo   ?? computeSEOScore(data.title,data.descLength);
  const viralScore  = cachedScores?.viral ?? computeViralScore(data.title,data.descLength,seoScore);
  const thumbScore  = cachedScores?.thumb ?? computeThumbScore(data.videoId);
  if(!cachedScores) setCachedAnalysis(data.videoId,{seo:seoScore,viral:viralScore,thumb:thumbScore});
  ...
  const checklist =buildChecklist(data.title,data.descLength,data.description);
  ...
  panel.innerHTML=`
    <div class="echo-header">
      <div class="echo-header-left">
        ...
        <div style="min-width:0"><h1 style="white-space:nowrap">VidSpark AI</h1><p>YouTube SEO Intelligence</p></div>
      </div>
  ...
  /* ── Adaptive Color : extraire couleurs miniature ── */
  (()=>{
    const vid = data?.videoId;
    if(vid){
      extractThumbnailColors(vid, colors => {
        if(colors) applyAdaptiveColor(colors);
      });
    }
  })();

  /* ── Badge plan dans le header (sans doublon de boutons) ── */
  (()=>{
    const hdrRight = panel.querySelector(".echo-header-right");
    if(!hdrRight) return;
    const langSel = hdrRight.querySelector(".echo-lang-select");
    if(!langSel) return;
    const badgeDiv = document.createElement("div");
    if(currentUser){
      /* Connecté: avatar + plan */
      badgeDiv.className = "echo-hdr-user";
      badgeDiv.id = "btnHdrUser";
      const initial = (currentUser.name||currentUser.email||"U")[0].toUpperCase();
      badgeDiv.innerHTML =
        '<span class="echo-hdr-avatar">'+initial+'</span>'
        +'<span class="echo-hdr-plan '+currentPlan+'">'+T("plan_"+currentPlan)+'</span>';
      hdrRight.insertBefore(badgeDiv, langSel);
    } else {
      /* Non connecté: badge plan simple */
      badgeDiv.className = "echo-plan-badge "+currentPlan;
      badgeDiv.textContent = T("plan_"+currentPlan);
      hdrRight.insertBefore(badgeDiv, langSel);
    }
  })();

  /* ── Ajouter la barre de quota pour FREE ── */
  (()=>{
    const quotaHtml=buildQuotaBar();
    if(quotaHtml){
      const header=panel.querySelector(".echo-header");
      if(header){
        const quotaDiv=document.createElement("div");
        quotaDiv.innerHTML=quotaHtml;
        header.parentNode.insertBefore(quotaDiv.firstElementChild, header.nextSibling);
      }
    }
  })();
...
function bindTabEvents(tab,data,scores,checklist){
  ...
  /* Avatar utilisateur dans le HEADER (connecté) */
  content.querySelector("#btnHdrUser")?.addEventListener("click",()=>{
    /* Clic sur l'avatar → menu rapide : déconnexion */
    const existing=document.getElementById("echo-hdr-menu");
    if(existing){existing.remove();return;}
    const menu=document.createElement("div");
    menu.id="echo-hdr-menu";
    const panelEl=document.getElementById("echo-rank-panel");
    const rect=content.querySelector("#btnHdrUser").getBoundingClientRect();
    menu.innerHTML=`
      <div class="echo-hdr-menu-item">${currentUser?.name||currentUser?.email||"Profil"}</div>
      <div class="echo-hdr-menu-sep"></div>
      <div class="echo-hdr-menu-item echo-hdr-menu-plan">${T("plan_"+currentPlan)} — ${
        currentPlan==="free"?(T("auth_free_plan")||"5/jour"):(T("auth_pro_plan")||"Illimité")}</div>
      ${currentPlan==="free"?`<div class="echo-hdr-menu-item echo-hdr-menu-upgrade" id="menuUpgrade">${T("upgrade_btn")||"Passer à Pro →"}</div>`:""}
      <div class="echo-hdr-menu-sep"></div>
      <div class="echo-hdr-menu-item echo-hdr-menu-logout" id="menuLogout">${T("btn_logout")||"Déconnexion"}</div>`;
    ...
    menu.querySelector("#menuLogout")?.addEventListener("click",async()=>{
      ...
      try{
        await new Promise((res,rej)=>{
          chrome.runtime.sendMessage({type:"VIDSPARK_AUTH",payload:{action:"logout"}},r=>{
            if(chrome.runtime.lastError)return rej();res(r);
          });
        });
      }catch(e){}
      currentUser=null;currentPlan="free";
      chrome.storage.local.remove(["accessToken","refreshToken","tokenExpiry","userPlan","userData","planCache","planCacheTime"]);
      showToast("✓ "+(T("btn_logout")||"Déconnecté"));
      panelMounted=false;currentVideoId=null;createPanel();
    });
    menu.querySelector("#menuUpgrade")?.addEventListener("click",()=>{menu.remove();showPremiumGate();});
  ...
  /* Bouton auth — Google OAuth */
  content.querySelector("#btnOvLogin")?.addEventListener("click",()=>showLoginModal());

  /* Bouton upgrade dans l'overview */
  content.querySelectorAll(".echo-upgrade-btn").forEach(btn=>{
  ...
  content.querySelector("#btnLoadTitles")?.addEventListener("click",async()=>{
    ...
    try{
      ...
      const typeLabels=[T("titles_seo"),T("titles_ctr"),T("titles_viral"),T("titles_shorts"),T("titles_trending")];
      const html=(res.titles||[]).map((t,i)=>`
        <div class="echo-title-result" data-title="${esc(t.text)}" style="background:var(--surface2);border:1px solid var(--border2);border-radius:10px;padding:10px 12px;margin-bottom:6px">
          <div style="font-size:10px;font-weight:700;color:var(--purpleL);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">${typeLabels[i]||""}</div>
          <div style="font-size:13px;font-weight:600;color:var(--text);margin-bottom:6px;line-height:1.4">${esc(t.text)}</div>
          <div style="display:flex;align-items:center;justify-content:space-between;gap:6px">
            <div style="display:flex;gap:4px">
              ...
              ${t.hook?`<span class="echo-badge echo-badge-ai">${esc(t.hook)}</span>`:""}
            </div>
            <button class="echo-copy-mini" style="background:var(--surface);border:1px solid var(--border2);color:var(--muted);font-size:11px;padding:3px 10px;border-radius:6px;cursor:pointer">📋 ${T("act_copy_title")||"Copier"}</button>
          </div>
        </div>`).join("")||`<div style="color:var(--muted);font-size:12px">${T("error_generic")}</div>`;
      setContent("card-titles-content",`
        <div class="echo-card-head">${T("nav_titles")} <span class="echo-badge echo-badge-ai">IA</span></div>
        ${html}
        <button class="echo-action-btn purple" id="btnRegenTitles" style="margin-top:4px">🔄 ${T("titles_generate")}</button>
      `);
      content.querySelectorAll(".echo-copy-mini").forEach(b=>{
        b.addEventListener("click",function(){
          navigator.clipboard.writeText(this.closest(".echo-title-result").dataset.title).then(()=>{
      ...
      content.querySelector("#btnRegenTitles")?.addEventListener("click",()=>{
        content.querySelector("#btnLoadTitles")?.click();
      });
...
/* ══════════════════════════════════════════════════════════════
   BOOTSTRAP — Vérification authentification Google
══════════════════════════════════════════════════════════════ */

chrome.storage.local.get(
  ["echoLanguage", "userPlan", "userData", "accessToken", "onboardingComplete", "primaryChannelId"],
  async (result) => {
    if (result.echoLanguage) currentLanguage = result.echoLanguage;

    // Vérifier authentification complète
    const isAuthenticated = result.accessToken && result.onboardingComplete && result.primaryChannelId;

    if (!isAuthenticated) {
      // ⛔ Pas authentifié : afficher message et ouvrir onboarding
      console.log("[VidSpark] User not authenticated - showing onboarding");

      // Afficher un message sur la page
      const overlay = document.createElement("div");
      overlay.id = "vidspark-auth-overlay";
      ...
      logoDiv.style.cssText = "font-size: 48px; margin-bottom: 20px;";
      logoDiv.textContent = "✨";

      const titleDiv = document.createElement("h2");
      titleDiv.style.cssText = "color: #ffffff; font-size: 24px; margin-bottom: 12px; font-weight: 700;";
      titleDiv.textContent = "Authentication Required";

      const descDiv = document.createElement("p");
      descDiv.style.cssText = "margin-bottom: 24px; line-height: 1.6;";
      descDiv.textContent = "VidSpark AI requires authentication with your Google account to get started.";

      const setupButton = document.createElement("button");
      setupButton.id = "vidspark-setup-btn";
      setupButton.style.cssText = `
        ...
        transition: all 0.3s;
      `;
      setupButton.textContent = "Open VidSpark Setup";

      // Add event listener BEFORE appending
      setupButton.addEventListener('click', (e) => {
        console.log('[VidSpark Auth Overlay] Setup button clicked');
        e.preventDefault();
        // Send message to background script to open the popup
        chrome.runtime.sendMessage(
          { type: 'VIDSPARK_OPEN_ONBOARDING' },
          (response) => {
            console.log('[VidSpark Auth Overlay] Background response:', response);
          }
        );
      });

      modalDiv.appendChild(logoDiv);
      modalDiv.appendChild(titleDiv);
      modalDiv.appendChild(descDiv);
      modalDiv.appendChild(setupButton);
      overlay.appendChild(modalDiv);
      document.body.appendChild(overlay);

      console.log('[VidSpark Auth] Overlay displayed');

      // Ouvrir l'onboarding dans une popup
    ...
    // ✅ Authentifié : charger normalement
    console.log("[VidSpark] User authenticated - loading extension");

    /* Vérifier le plan côté serveur */
    if (result.accessToken) {
      try {
        const planData = await new Promise((res, rej) => {
          chrome.runtime.sendMessage({ type: "VIDSPARK_CHECK_PLAN" }, (r) => {
        ...
        if (planData && !planData.error) {
          ...
          currentMonthlyUsage = planData.monthly_usage || 0;
          currentMonthlyLimit = planData.monthly_limit || 10;
          chrome.storage.local.set({
            userPlan: currentPlan,
            monthlyUsage: currentMonthlyUsage,
            monthlyLimit: currentMonthlyLimit
          });
        }
      } catch (e) {
        console.error("[VidSpark] Plan check error:", e);
        currentPlan = result.userPlan || "free";
        currentMonthlyUsage = result.monthlyUsage || 0;
        currentMonthlyLimit = result.monthlyLimit || 10;
      }

      currentUser = result.userData || null;
    } else {
      currentPlan = "free";
      currentUser = null;
    }

    /* Créer et mettre à jour le panel */
    createPanel();
    setTimeout(createPanel, 2500);
    setTimeout(createPanel, 5000);

    /* Recharger panel quand URL change */
    setInterval(() => {
