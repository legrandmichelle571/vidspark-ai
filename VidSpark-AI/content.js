...
function renderActivationHTML(){
  ...
  return `
    ...
    <!-- Connexion automatique : clic → ouverture du site → handshake site⇄extension
         (background.js) → connecté, sans aucune saisie. Code de secours à UN SEUL
         champ si la détection/handshake échoue (remplace l'ancien ID+Secret). -->
    <div id="connectProgress" style="display:none;text-align:center;padding:10px 0 6px;">
      <div style="width:26px;height:26px;margin:0 auto 12px;border:3px solid #2a2a35;border-top-color:#7c6dfa;border-radius:50%;animation:vs-spin 0.8s linear infinite;"></div>
      <p style="font-size:13px;color:#fff;font-weight:600;margin:0;">${esc(currentLanguage==='fr'?"Connexion à VidSpark…":"Connecting to VidSpark…")}</p>
      <p style="font-size:11px;color:#aaa;margin:6px 0 12px;">${esc(currentLanguage==='fr'?"Ouverture du site dans un nouvel onglet…":"Opening the site in a new tab…")}</p>
    </div>
    <style>@keyframes vs-spin{to{transform:rotate(360deg)}}</style>

    <div id="connectErrorBlock" style="display:none;text-align:center;padding:6px 0;">
      <p style="font-size:13px;color:#fa6d6d;font-weight:600;margin:0 0 12px;">${esc(currentLanguage==='fr'?"Impossible de terminer la connexion.":"Could not complete the connection.")}</p>
      <button id="btnRetryConnect" style="width:100%;padding:12px;background:#7c6dfa;color:#fff;border:none;border-radius:8px;font-weight:bold;font-size:13px;cursor:pointer;margin-bottom:8px;">🔄 ${esc(currentLanguage==='fr'?"Réessayer":"Retry")}</button>
    </div>

    <div id="connectFallbackBlock" style="display:none;">
      <p style="font-size:13px;color:#fff;font-weight:600;text-align:center;margin:0 0 4px;">${esc(currentLanguage==='fr'?"Extension non détectée.":"Extension not detected.")}</p>
      <p style="font-size:11px;color:#aaa;text-align:center;margin:0 0 12px;">${esc(currentLanguage==='fr'?"Vérifie que VidSpark AI est bien installée et active.":"Make sure VidSpark AI is installed and enabled.")}</p>
      <button id="btnRetryDetect" style="width:100%;padding:12px;background:#7c6dfa;color:#fff;border:none;border-radius:8px;font-weight:bold;font-size:13px;cursor:pointer;margin-bottom:8px;">🔄 ${esc(currentLanguage==='fr'?"Réessayer":"Retry")}</button>
      <button id="btnUseCode" type="button" style="width:100%;background:transparent;border:none;color:#8b8b98;font-size:11px;cursor:pointer;text-decoration:underline;">${esc(currentLanguage==='fr'?"Utiliser un code de connexion":"Use a connection code")}</button>

      <div id="codeRedeemBlock" style="display:none;margin-top:14px;">
        <label style="display:block;font-size:11px;color:#aaa;margin-bottom:4px;text-align:left;">${esc(currentLanguage==='fr'?"Code de connexion":"Connection code")}</label>
        <input type="text" id="pairCodeInput" placeholder="A7K9X-2PMQR" autocapitalize="characters" autocomplete="off" style="width:100%;padding:10px;border:1px solid #7c6dfa;border-radius:8px;background:#0f0f1a;color:#fff;font-size:12px;box-sizing:border-box;text-align:center;letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;">
        <button id="btnRedeemCode" style="width:100%;padding:12px;background:#7c6dfa;color:white;border:none;border-radius:8px;font-weight:bold;font-size:13px;cursor:pointer;margin-bottom:8px;">✅ ${esc(currentLanguage==='fr'?"Valider":"Validate")}</button>
        <p id="activationStatus" style="font-size:11px;color:#aaa;min-height:14px;margin-bottom:4px;"></p>
      </div>
    </div>

    <button id="btnStartConnect" style="width:100%;padding:12px;background:#7c6dfa;color:white;border:none;border-radius:8px;font-weight:bold;font-size:13px;cursor:pointer;transition:all 0.2s;margin-bottom:8px;">🔑 ${esc(currentLanguage==='fr'?"Connexion":"Connect")}</button>

    <!-- Connexion Google native (chantier B) — alternative sans passer par le site,
         conservée telle quelle (seul appelant réel de POST /api/auth/google). -->
    <button id="googleSignInBtn" type="button" style="width:100%;padding:11px;background:#fff;color:#1a1a1a;border:none;border-radius:8px;font-weight:700;font-size:12.5px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:8px;">
      ...
      <span id="googleSignInLabel">${esc(currentLanguage==='fr'?"Continuer avec Google":"Continue with Google")}</span>
    </button>
...
function wireActivationEvents(container, opts = {}){
  ...
  const startBtn = container.querySelector('#btnStartConnect');
  const progressBlock = container.querySelector('#connectProgress');
  const errorBlock = container.querySelector('#connectErrorBlock');
  const fallbackBlock = container.querySelector('#connectFallbackBlock');
  const codeBlock = container.querySelector('#codeRedeemBlock');
  ...
  function showSub(sub){
    startBtn.style.display = sub ? 'none' : '';
    if (googleBtn) googleBtn.style.display = sub ? 'none' : '';
    progressBlock.style.display = sub === 'progress' ? '' : 'none';
    errorBlock.style.display    = sub === 'error'    ? '' : 'none';
    fallbackBlock.style.display = sub === 'fallback'  ? '' : 'none';
    if (sub !== 'fallback' && codeBlock) codeBlock.style.display = 'none';
  }
  ...
  function onStorageChange(changes, area){
    ...
    chrome.storage.local.get(['activation_id','activation_secret','subscription_expiry','userEmail','userName','userPlan'], (data) => {
      if (!data.activation_id || !data.activation_secret) return;
      currentUserEmail = data.userEmail || currentUserEmail;
      currentPlan = data.userPlan || currentPlan || 'free';
      panelMounted = false;
      panelCreating = false;
      onSuccess();
    });
  ...
  startBtn.addEventListener('click', startAutoConnect);
  container.querySelector('#btnRetryConnect').addEventListener('click', startAutoConnect);
  container.querySelector('#btnRetryDetect').addEventListener('click', startAutoConnect);

  if (googleBtn) {
    googleBtn.addEventListener('click', () => {
      googleBtn.disabled = true;
      const label = container.querySelector('#googleSignInLabel');
      if (label) label.textContent = currentLanguage==='fr' ? 'Connexion…' : 'Signing in…';
      googleStatus.textContent = '';
      chrome.runtime.sendMessage({ type: 'VIDSPARK_GOOGLE_SIGNIN' }, (resp) => {
        googleBtn.disabled = false;
        if (label) label.textContent = currentLanguage==='fr' ? 'Continuer avec Google' : 'Continue with Google';
        ...
        chrome.storage.local.get(['userEmail','userPlan'], (data) => {
          currentUserEmail = data.userEmail || currentUserEmail;
          currentPlan = data.userPlan || currentPlan || 'free';
          panelMounted = false;
          panelCreating = false;
  ...
  container.querySelector('#btnUseCode').addEventListener('click', () => {
    clearTimeout(connectTimer);
    chrome.storage.onChanged.removeListener(onStorageChange);
    codeBlock.style.display = codeBlock.style.display === 'none' ? 'block' : 'none';
    if (codeBlock.style.display === 'block') container.querySelector('#pairCodeInput').focus();
  });

  container.querySelector('#btnRedeemCode').addEventListener('click', () => {
    const code = container.querySelector('#pairCodeInput').value.trim();
    if (!code) {
      statusEl.textContent = '⚠️ ' + (currentLanguage==='fr'?"Entre le code de connexion.":"Enter the connection code.");
      statusEl.style.color = '#fa6d6d';
      return;
    }
    statusEl.textContent = (currentLanguage==='fr'?"Vérification en cours…":"Checking…");
    statusEl.style.color = '#aaa';
    chrome.runtime.sendMessage({ type: 'VIDSPARK_PAIR_REDEEM', code }, (resp) => {
      if (chrome.runtime.lastError || !resp || !resp.success) {
        statusEl.textContent = '❌ ' + (currentLanguage==='fr'?"Code invalide ou expiré.":"Invalid or expired code.");
        statusEl.style.color = '#fa6d6d';
        return;
      }
  ...
  (async () => {
    try {
      if (result.activation_id && result.activation_secret) {
        ...
        const r = await fetch('https://vidspark-ai-production-9ac7.up.railway.app/api/auth/activation/activate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
